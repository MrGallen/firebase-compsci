...
A simple python wrapper for the [Firebase API](https://firebase.google.com) based of Pyrebase 4 and python-firebase for Leaving Certificate Computer Science

First part of tutorial is for python-firebase package and second part is for use with pyrebase
...
```python
from compscifirebase import firebase
from compscifirebase import pyrebase
```


## Getting Started - Python-Firebase
=================
...
```python
    ...
    firebasedb1 = firebase.FirebaseApplication('https://your_storage.firebaseio.com', None)
    result = firebasedb1.get('/users', None)
    print(result)
...
```python
    from compscifirebase import firebase
    firebasedb1 = firebase.FirebaseApplication('https://your_storage.firebaseio.com', None)
    result = firebasedb1.get('/users', '1')
    print(result)
    {'1': 'John Doe'}
```
...
```python
    from compscifirebase import firebase
    firebasedb1 = firebase.FirebaseApplication('https://your_storage.firebaseio.com', None)
    result = firebasedb1.get('/users/2', None, {'print': 'pretty'}, {'X_FANCY_HEADER': 'VERY FANCY'})
    print(result)
    {'2': 'Jane Doe'}
```
...
```python
    ...
    result = firebasedb1.post('/users', new_user, {'print': 'pretty'}, {'X_FANCY_HEADER': 'VERY FANCY'})
    print(result)
    {u'name': u'-Io26123nDHkfybDIGl7'}

    result = firebasedb1.post('/users', new_user, {'print': 'silent'}, {'X_FANCY_HEADER': 'VERY FANCY'})
    print(result == None)
    True
```
...
```python
    from compscifirebase import firebase
    firebasedb1 = firebase.FirebaseApplication('https://your_storage.firebaseio.com', authentication=None)
    result = firebasedb1.get('/users', None, {'print': 'pretty'})
    print(result)
...
.. code-block:: python
     ...
     import json
     from compscifirebase import firebase
     from compscifirebase import jsonutil
